...
class Upload{
	 ...
	 */
	public function up($file, $folder){
		...
		*/
		...
		if ($file['error'] == 0) {
			...
			$file_name = $name.strrchr($file['name'], '.');
			$filemime = strrchr($file['name'], '.');
	 ...
	 */
	public function multiUp($files, $folder){
